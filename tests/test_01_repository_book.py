...
from book_library.repository import RepositoryTask
...
class TestRepository:
    ...
    # def test_create_task(self, repository: RepositoryTask, task: BookSchema):
    #     task_data = task.model_dump()
    #     task_data.pop("id")

    #     new_task = repository.create(**task_data)

    #     assert isinstance(new_task, BookSchema), (
    ...
    #             f"После создания задачи в объекте {BookSchema.__name__} "
    ...
    #     )
    #     assert all(
    #         map(lambda item: isinstance(item, BookSchema), tasks)
    #     ), f"Все задачи должны быть типом {BookSchema.__name__}."
    ...
    #     assert isinstance(get_task, BookSchema), (
